...
class VideoProcessor:
    ...
    def calculate_scaled_coordinates(self, box, original_height, original_width):
        """
        Converts a detection box
        – Computes its center and dimensions,
        – Applies an upward offset (10% of box height) so that head is in view,
        – Maps the center from [0, original_width/original_height] to a coordinate system ranging
        from -target_width/2 to target_width/2 and likewise for height,
        – Clamps the resulting position so that when applied in revideo the video is not shifted too far.
        """
        x1, y1, x2, y2 = box
        orig_width_box = x2 - x1
        orig_height_box = y2 - y1

        # Skip very small detections.
        min_width = original_width * 0.05
        min_height = original_height * 0.05
        if orig_width_box < min_width or orig_height_box < min_height:
            return None
        ...
        adjusted_center_y = center_y - (0.1 * orig_height_box)

        # Map the center coordinates to a coordinate system from -target_dim/2 to target_dim/2.
        ...
        # Now, clamp the mapped position.
        # Compute the maximum allowed shift so the video (of size target_width x target_height) still covers the layout.
        ...
        # Also compute the scaled size of the detection box within the new coordinate system.
        scaled_width = orig_width_box * (self.target_width / original_width)
        scaled_height = orig_height_box * (self.target_height / original_height)
        return {
